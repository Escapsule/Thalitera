...
const ManageBookingPage = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      // Import mock data
      const { bookings: mockBookings } = await import('@/lib/admin/bookingMock_data')
      
      // Use type assertion to ensure type matching
      setBookings(mockBookings as Booking[])
      setFilteredBookings(mockBookings as Booking[])
    } catch (error) {
      console.error('Failed to retrieve booking records:', error)
      // If import fails, use an empty array
      setBookings([])
      setFilteredBookings([])
    } finally {
      setLoading(false)
  ...
  const applyFilters = () => {
    ...
    if (filters.date) {
      result = result.filter(booking => 
        new Date(booking.start_time).toISOString().split('T')[0] === filters.date
      )
    }
    ...
    if (filters.startTime && filters.endTime) {
      ...
      result = result.filter(booking => {
        ...
        return (
          (bookingStartTime <= filterEndTime && bookingEndTime >= filterStartTime) ||
          (filterStartTime <= bookingEndTime && filterEndTime >= bookingStartTime)
        )
      })
    }
    ...
    if (filters.search) {
      ...
      result = result.filter(booking => 
        ...
        booking.user_id.toLowerCase().includes(searchLower)
      )
    }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="filter-reservation-id">Booking ID</Label>
                  <div className="relative">
                    <Search className="absolute left-2 top-2.5 h-4 w-4 text-gray-500" />
                    <Input
                      id="filter-reservation-id"
                        ...
                        <TableHead className="w-[15%]">
                          <div className="flex items-center justify-between">
                            ...
                            <Button
                              ...
                              onClick={() => setSortByReservationId(!sortByReservationId)}
                            >
                              <ArrowUpDown 
                                className={`h-4 w-4 ${sortByReservationId ? 'text-primary' : 'text-gray-400'}`}
                              />
                      ...
                      {currentBookings.length === 0 ? (
                        ...
                        currentBookings.map((booking) => (
                          <TableRow key={booking.reservation_id}>
                            <TableCell className="font-medium">{booking.reservation_id}</TableCell>
            ...
            <div className="py-4">
              ...
              <div className="space-y-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-gray-500 mb-3">Meeting Room Information</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm text-gray-500">Meeting Room Name</label>
                      <p className="font-medium">{selectedBooking.room_name}</p>
                    </div>
                    <div>
                      <label className="text-sm text-gray-500">Meeting Room ID</label>
                      <p className="font-medium">{selectedBooking.room_id}</p>
                    </div>
                  </div>
                </div>

                {/* Booking User Information Section */}
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h3 className="text-sm font-semibold text-gray-500 mb-3">Booking User Information</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="font-medium">{selectedBooking.reservation_id}</p>
                    </div>
                    <div>
                      <label className="text-sm text-gray-500">Booking Time</label>
                      <p className="font-medium">
                        {format(new Date(selectedBooking.start_time), 'yyyy-MM-dd HH:mm')} - 
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {format(new Date(selectedBooking.created_at), 'yyyy-MM-dd HH:mm:ss')}
                      </p>
                    </div>
                    <div>
                      <label className="text-sm text-gray-500">Updated Time</label>
                      <p className="font-medium">
                        {format(new Date(selectedBooking.updated_at), 'yyyy-MM-dd HH:mm:ss')}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {selectedBooking.attendees && selectedBooking.attendees.length > 0 ? (
                          selectedBooking.attendees.map((attendeeId, index) => (
                            <Badge 
                              key={index} 
                              variant="outline" 
                              className="px-2 py-1"
                          ...
                          <p className="text-sm text-gray-500 italic">There are currently no attendees</p>
                        )}
                      </div>
                    </div>
                  </div>
        ...
        <DialogContent>
          ...
          <div className="py-4">
            ...
            <div className="mt-4 p-4 bg-gray-50 rounded-md">
              <p><strong>Booking ID:</strong> {selectedBooking?.reservation_id}</p>
              <p><strong>Meeting Room:</strong> {selectedBooking?.room_name}</p>
              <p><strong>Booking User:</strong> {selectedBooking?.user_name}</p>
              <p><strong>Booking Time:</strong> {selectedBooking && 
                `${format(new Date(selectedBooking.start_time), 'yyyy-MM-dd HH:mm')} - 
                 ${format(new Date(selectedBooking.end_time), 'HH:mm')}`}</p>
            </div>
            <div className="mt-4">
